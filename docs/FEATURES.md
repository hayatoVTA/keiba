...
**注意**: アカウント登録は必須です。ゲストモードは提供していません。
...
#### 馬連
- **説明**: 1着・2着の組み合わせを予想（順番は問わない）
- **選択数**: 2頭
- **配当計算**: 賭け金 × 馬連オッズ
- **特徴**: 配当が大きい

#### ワイド
- **説明**: 3着以内の2頭の組み合わせを予想（順番は問わない）
- **選択数**: 2頭
- **配当計算**: 賭け金 × ワイドオッズ
- **特徴**: バランス型、的中しやすい

#### 3連複
- **説明**: 3着以内の3頭の組み合わせを予想（順番は問わない）
- **選択数**: 3頭
- **配当計算**: 賭け金 × 3連複オッズ
- **特徴**: 高配当、順番は関係ないので的中しやすい

#### 3連単
- **説明**: 1着・2着・3着の順番を正確に予想
- **選択数**: 3頭（順番指定）
- **配当計算**: 賭け金 × 3連単オッズ
- **特徴**: 最高配当、順番を正確に当てる必要がある
...
interface Bet {
  ...
  raceId: string;                 // レースID
  betType: 'win' | 'place' | 'exacta' | 'wide' | 'trio' | 'trifecta';
  // win: 単勝, place: 複勝, exacta: 馬連, wide: ワイド
...
1. **レース選択**: レース一覧から選択
2. **予想タイプ選択**: 単勝、複勝、馬連、ワイド、3連複、3連単から選択
3. **馬選択**: 出走馬から選択
   - 単勝・複勝: 1頭
   - 馬連・ワイド: 2頭
   - 3連複: 3頭（順番は問わない）
...
## 7. 将来実装予定機能（ゲーミフィケーション）

MVPでは実装しませんが、製品版以降で実装予定のゲーミフィケーション機能です。詳細は [将来実装予定機能](./FUTURE_FEATURES.md) を参照してください。
